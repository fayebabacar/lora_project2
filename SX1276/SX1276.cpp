...
/*  Added SX1276 functionality by modifying original SX1272 code 
 *  for bug fixes please contact Aurelian Lazarut (Adaptive Design ltd.)
 *  This library is assuming the RF output pin is the Boost pin (not the RFO pin) and is 
 *  compatible with the Adaptive Design LoRa Shield
 */
...
// Added by C. Pham
// based on SIFS=3CAD
uint8_t sx1276_SIFS_value[11]={0, 183, 94, 44, 47, 23, 24, 12, 12, 7, 4};
uint8_t sx1276_CAD_value[11]={0, 62, 31, 16, 16, 8, 9, 5, 3, 1, 1};
// end
//**********************************************************************/
// Public functions.
//**********************************************************************/
...
*/
uint8_t SX1276::setLORA() //NOT MODIFIED
{
    uint8_t state = 2;
    byte st0;
...
uint8_t SX1276::getMode()
{
  ...
  _bandwidth = (value >> 4);   			// Storing 2 MSB from REG_MODEM_CONFIG1 (=_bandwidth)
  _codingRate = (value >> 1) & 0x07;  		// Storing third, forth and fifth bits from
...
int8_t	SX1276::setHeaderOFF()
{
  ...
  {
	  config1 = readRegister(REG_MODEM_CONFIG1);	// Save config1 to modify only the header bit
	  config1 = config1 | B00000001;				// sets bit 2 from REG_MODEM_CONFIG1 = headerOFF
	  writeRegister(REG_MODEM_CONFIG1,config1);		// Update config1
...
uint8_t	SX1276::setCRC_ON()
{
  ...
  { // LORA mode
	...
	config2 = config2 | B00000100;				// sets bit 2 from REG_MODEM_CONFIG2 = CRC_ON
...
uint8_t	SX1276::setCRC_OFF()
{
  ...
  { // LORA mode
  	config2 = readRegister(REG_MODEM_CONFIG2);	// Save config1 to modify only the CRC bit
	config2 = config2 & B11111011;				// clears bit 1 from config1 = CRC_OFF
	writeRegister(REG_MODEM_CONFIG2,config2);
...
uint8_t	SX1276::setSF(uint8_t spr)
{
  ...
  { // LoRa mode
	...
	switch(spr)	 
	{
		case SF_6: 	config2 = config2 & SF_MASK;	// clears bits 7 & 4 from REG_MODEM_CONFIG2
					config2 = config2 | SF_6;	// sets bits 6 & 5 from REG_MODEM_CONFIG2
					setHeaderOFF();		// Mandatory headerOFF with SF = 6
					break;
		case SF_7: 	config2 = config2 & SF_MASK;	// clears bits 7 from REG_MODEM_CONFIG2
					config2 = config2 | SF_7;	// sets bits 6, 5 & 4
					break;
		case SF_8: 	config2 = config2 & SF_MASK;	// clears bits 6, 5 & 4 from REG_MODEM_CONFIG2
					config2 = config2 | SF_8;	// sets bit 7 from REG_MODEM_CONFIG2
					break;
		case SF_9: 	config2 = config2 & SF_MASK;	// clears bits 6, 5 & 4 from REG_MODEM_CONFIG2
					config2 = config2 | SF_9;	// sets bits 7 & 4 from REG_MODEM_CONFIG2
					break;
		case SF_10:	config2 = config2 & SF_MASK;	// clears bits 6 & 4 from REG_MODEM_CONFIG2
					config2 = config2 | SF_10;	// sets bits 7 & 5 from REG_MODEM_CONFIG2
					break;
		case SF_11:	config2 = config2 & SF_MASK;	// clears bit 6 from REG_MODEM_CONFIG2
					config2 = config2 | SF_11;	// sets bits 7, 5 & 4 from REG_MODEM_CONFIG2
					getBW();
					...
					break;
		case SF_12: config2 = config2 & SF_MASK;	// clears bits 5 & 4 from REG_MODEM_CONFIG2
					config2 = config2 | SF_12;	// sets bits 7 & 6 from REG_MODEM_CONFIG2
					if( _bandwidth == BW_125 )
	...
	config3 = (readRegister(REG_MODEM_CONFIG3));	// Save config3 to check update
...
int8_t	SX1276::setBW(uint16_t band)
{
  ...
  config3 = (readRegister(REG_MODEM_CONFIG3));	// Save config1 to modify only the BW
  switch(band)
  {
	  case BW_125:  config1 = config1 & BW_MASK;	// clears bits 7 & 6 from REG_MODEM_CONFIG1
					config1 = config1 | BW_125;
					getSF();
					...
					break;
	  case BW_250:  config1 = config1 & BW_MASK;	// clears bit 7 from REG_MODEM_CONFIG1
					config1 = config1 | BW_250;	// sets bit 6 from REG_MODEM_CONFIG1
					break;
	  case BW_500:  config1 = config1 & BW_MASK;	//clears bit 6 from REG_MODEM_CONFIG1
					config1 = config1 | BW_500;	//sets bit 7 from REG_MODEM_CONFIG1
					break;
  }
  writeRegister(REG_MODEM_CONFIG1,config1);		// Update config1
  writeRegister(REG_MODEM_CONFIG3,config3);		// Update config1
  ...
  switch(band)
  {
	   case BW_125: if( (config1 & 0xF0) == BW_125 )
					{
						state = 0;
						if( _spreadingFactor == 11 )
						{
					...
					break;
	   case BW_250: if( (config1 & 0xF0) == BW_250 )
					{
						state = 0;
					}
					break;
	   case BW_500: if( (config1 & 0xF0) == BW_500 )
					{
						state = 0;
					}
					break;
...
int8_t	SX1276::setCR(uint8_t cod)
{
  ...
  switch(cod)
  {
	 case CR_5: config1 = config1 & CR_MASK;	// clears bits 5 & 4 from REG_MODEM_CONFIG1
				config1 = config1 | CR_5;	// sets bit 3 from REG_MODEM_CONFIG1
				break;
	 case CR_6: config1 = config1 & CR_MASK;	// clears bits 5 & 3 from REG_MODEM_CONFIG1
				config1 = config1 | CR_6;	// sets bit 4 from REG_MODEM_CONFIG1
				break;
	 case CR_7: config1 = config1 & CR_MASK;	// clears bit 5 from REG_MODEM_CONFIG1
				config1 = config1 | CR_7;	// sets bits 4 & 3 from REG_MODEM_CONFIG1
				break;
	 case CR_8: config1 = config1 & CR_MASK;	// clears bits 4 & 3 from REG_MODEM_CONFIG1
				config1 = config1 | CR_8;	// sets bit 5 from REG_MODEM_CONFIG1
				break;
  }
  writeRegister(REG_MODEM_CONFIG1, config1);		// Update config1
...
*/
int8_t SX1276::setPower(char p) // NOT MODIFIED, SHOULD HAVE BEEN
{
  byte st0;
  int8_t state = 2;
  byte value = 0x00;
...
void SX1276::setPacketType(uint8_t type)
{
...
/*
 ...
 Returns: Integer that determines if the number of requested CAD have been successfull
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t SX1276::doCAD(uint8_t counter)
{
    uint8_t state = 2;
    byte value = 0x00;
...
#ifdef DEBUG_CAD
    Serial.println(F("SX1276::Starting 'doCAD'"));
#endif
        ...
        do {
            ...
            if( bitRead(value, 2) == 1 )
            {
...
#ifdef DEBUG_CAD				  
                Serial.print(F("SX1276::CAD duration "));
                Serial.println(endCAD-startCAD);
                Serial.println(F("SX1276::CAD successfully performed"));
#endif				  
                ...
                if( bitRead(value, 0) == 1 )
                {
                    ...
                    failedCAD=true;
#ifdef DEBUG_CAD				  		
                    Serial.print(F("SX1276::CAD exits after "));
            ...
            else
            {
                ...
                {
#ifdef DEBUG_CAD
                    Serial.println(F("SX1276::Timeout has expired"));
#endif
                }
                else
                {
#ifdef DEBUG_CAD
                    Serial.println(F("SX1276::Error and CAD has not been performed"));
#endif
                }
...
#ifdef DEBUG_CAD	  
    Serial.print(F("SX1276::doCAD duration "));
...
uint16_t SX1276::getToA(uint8_t pl) {
...
#ifdef DEBUG_GETTOA
    Serial.print(F("SX1276::bw is "));
    Serial.println(bw);

    Serial.print(F("SX1276::SF is "));
...
#ifdef DEBUG_GETTOA	
    Serial.print(F("SX1276::ts is "));
    printDouble(ts,6);
    Serial.println();
    Serial.print(F("SX1276::tPreamble is "));
...
#ifdef DEBUG_GETTOA    
    Serial.print(F("SX1276::nPayload is "));
...
#ifdef DEBUG_GETTOA    
    Serial.print(F("SX1276::airTime is "));
            ...
            do {
                ...
                if (!e) {
                    ...
                    if (_extendedIFS)  {
                        ...
                        Serial.print(w);
                        Serial.print(F(" CAD = "));
                        Serial.print(sx1276_CAD_value[_loraMode]*w);
                        Serial.println();

                        delay(sx1276_CAD_value[_loraMode]*w);
                    ...
                    Serial.print(sx1276_SIFS_value[_loraMode]*3*w);
                    Serial.println();

                    delay(sx1276_SIFS_value[_loraMode]*3*w);
...
*/
int8_t	SX1276::setSyncWord(uint8_t sw)
{
    byte st0;
    int8_t state = 2;
    byte config1;
